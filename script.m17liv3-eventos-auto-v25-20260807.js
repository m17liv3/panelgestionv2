...
// ========== DETECCION AUTOMATICA DE EVENTOS PARA RESULTADOS ==========
...
function cartNormalizeSportLabel(text) {
  ...
  if (/beisbol|baseball|mlb/.test(t)) return 'Baseball';
  ...
  if (/futbol|football|soccer|champions|laliga|premier|serie a|bundesliga|mundial/.test(t)) return 'Football';
  return '';
}

function cartCleanTeamName(text) {
  return String(text || '')
    .replace(/^[\s\-–—|:;,.•·▪▫►▶◆◇★⭐⚽🏀🏈⚾🏒🤾🏐]+/g,'')
    .replace(/[\s\-–—|:;,.•·▪▫►▶◆◇★⭐⚽🏀🏈⚾🏒🤾🏐]+$/g,'')
    .replace(/\s+/g,' ')
...
function cartParseDailyEvents(text) {
  ...
  lines.forEach(function(raw, idx) {
    ...
    // Encabezados de deporte: actualizan el contexto y no cuentan como evento.
    var sportHint = cartNormalizeSportLabel(line);
    var hasVersus = /\s(?:vs\.?|v\.?|contra)\s/i.test(line) || /\s[-–—]\s/.test(line);
    if (sportHint && !hasVersus && line.length < 70) {
      currentSport = sportHint;
      return;
    }

    var timeMatch = line.match(/(?:^|\s)([0-2]?\d[:.][0-5]\d)(?:\s|$)/);
    var hora = timeMatch ? timeMatch[1].replace('.',':') : '';

    // Quitamos hora al principio y adornos habituales, pero conservamos el texto original.
    var work = line
      .replace(/^\s*[⏰🕐🕑🕒🕓🕔🕕🕖🕗🕘🕙🕚🕛]?\s*[0-2]?\d[:.][0-5]\d\s*[-–—|·:]?\s*/,'')
      .replace(/^\s*[•·▪▫►▶◆◇★⭐⚽🏀🏈⚾🏒🤾🏐]+\s*/,'')
      .trim();

    var sep = work.match(/\s(?:vs\.?|v\.?|contra)\s|\s[-–—]\s/i);
    if (!sep) return;

    var pos = sep.index;
    var left = work.slice(0,pos).trim();
    var right = work.slice(pos + sep[0].length).trim();

    // Si hay prefijo "Fútbol:" o una competición antes del equipo, nos quedamos con lo posterior.
    if (left.indexOf(':') >= 0) {
      var lp = left.split(':');
      if (lp.length > 1 && cartNormalizeSportLabel(lp[0])) left = lp.slice(1).join(':').trim();
    }

    // Retira hora y datos típicos posteriores (canal/carpeta) del visitante.
    right = right
      .replace(/\s+[|·]\s+.*$/,'')
      .replace(/\s+\(?[0-2]?\d[:.][0-5]\d\)?\s*$/,'')
      .replace(/\s+(?:canal|tv|carpeta)\s*[:=-].*$/i,'')
      .trim();

    left = cartCleanTeamName(left);
    right = cartCleanTeamName(right);

    // Evita falsos positivos de separadores usados como texto normal.
    if (left.length < 2 || right.length < 2 || left.length > 70 || right.length > 70) {
      rejected.push({ linea: idx + 1, texto: line });
      return;
    }

    var sport = currentSport || cartNormalizeSportLabel(line) || 'Unknown';
    var key = (sport + '|' + left + '|' + right).toLowerCase();
    if (seen[key]) return;
    seen[key] = true;
...
function cartRenderDetectedEvents(text) {
  ...
  if (!parsed.events.length) {
    list.innerHTML = '<div style="color:#FFB35C;font-size:12px;line-height:1.5;">No he detectado enfrentamientos todavía. Puedes publicar igualmente, pero no habrá cruce automático de resultados.</div>';
    return;
  }
  list.innerHTML = parsed.events.slice(0,40).map(function(e){
    var icon = e.sport === 'Basketball' ? '&#127936;' : e.sport === 'Baseball' ? '&#9918;' : e.sport === 'Hockey' ? '&#127954;' : e.sport === 'Handball' ? '&#129342;' : e.sport === 'Volleyball' ? '&#127952;' : '&#9917;';
    return '<div style="display:flex;gap:8px;align-items:flex-start;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05);font-size:12px;color:#DCEBFA;">' +
      ...
      '<span style="min-width:0;flex:1;"><b>' + cartEscapeHtml(e.home) + '</b> <span style="color:#6E8AA4;">vs</span> <b>' + cartEscapeHtml(e.away) + '</b>' +
      (e.time ? '<span style="color:#00D4FF;margin-left:7px;">' + cartEscapeHtml(e.time) + '</span>' : '') + '</span></div>';
  }).join('') + (parsed.events.length > 40 ? '<div style="padding-top:8px;color:#7AA3C8;font-size:11px;">+' + (parsed.events.length-40) + ' eventos más</div>' : '');
}
// ========== FIN DETECCION AUTOMATICA DE EVENTOS ==========
