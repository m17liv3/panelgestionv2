...
function cartParseDailyEvents(text) {
  ...
  lines.forEach(function(raw, idx) {
    ...
    // Un encabezado de deporte solo cambia el contexto.
    if (!/^\s*⏰/.test(line) && !/^\s*[0-2]?\d[:.][0-5]\d\b/.test(line)) {
      var headerSport = cartNormalizeSportLabel(line);
      if (headerSport && line.length < 90 && !/^📺|^🗂/.test(line)) currentSport = headerSport;
      return;
    }

    // Cualquier línea con hora de la cartelera cuenta como un evento real.
    ...
    var sport = currentSport || cartNormalizeSportLabel(title) || 'Unknown';
    var type = 'event';
    var home = '';
    var away = '';

    // Solo tratamos como enfrentamiento un separador inequívoco. Un guion normal
    // NO vale: en ciclismo, natación, golf, etc. forma parte del nombre del evento.
    var versus = title.match(/\s*(?:🆚|\bvs\.?\b|\bv\.?\b|\bcontra\b)\s*/i);
    if (versus) {
      var pos = versus.index;
      var left = title.slice(0, pos).trim();
      var right = title.slice(pos + versus[0].length).trim();

      // Prefijos habituales: "Pretemporada: UE Olot 🆚 Girona" / "Torneo: A 🆚 B".
      if (left.indexOf(':') >= 0) {
        var chunks = left.split(':');
        left = chunks[chunks.length - 1].trim();
      }
      // El texto de la cartelera puede llevar el canal/carpeta en la misma línea.
      // Esos metadatos NO forman parte del nombre del segundo participante.
      right = right
        .replace(/\s+[|·]\s+.*$/,'')
        .replace(/\s+(?:📺|🗂)\s*(?:CANAL|CARPETA)?\s*:?.*$/i,'')
        .trim();
    ...
    var key = (sport + '|' + hora + '|' + title).toLowerCase();
    if (seen[key]) return;
    seen[key] = true;
