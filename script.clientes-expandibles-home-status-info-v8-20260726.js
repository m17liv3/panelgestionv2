...
function navClients() {
  ...
  setTimeout(function(){ if (search) search.focus({preventScroll:true}); }, 180);
}
  ...
  filtered.forEach(function(c) {
    ...
    div.innerHTML =
      '<button type="button" class="clientCard-summary" data-id="'+esc(c.id)+'" aria-expanded="'+(isExpanded?'true':'false')+'" onclick="toggleClientCardExpanded(this.dataset.id)">' +
        '<span class="clientCard-avatar">' + esc(avatarLetter(c.name)) + '</span>' +
        ...
        '</span>' +
        '<span class="clientCard-summaryDate">' + (cardStatus === 'exp' ? 'Caducó ' : 'Caduca ') + formatDate(c.expiry) + '</span>' +
