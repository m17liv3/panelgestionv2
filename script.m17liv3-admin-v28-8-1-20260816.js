...
// Cierra cualquier ventana, ficha o panel antes de cambiar de sección desde
// la barra inferior. Así nunca quedan dos apartados abiertos a la vez.
var sheetOpenToken = 0;
function closeAllNavigationPanels() {
  sheetOpenToken += 1;

  try {
    Array.prototype.slice.call(document.querySelectorAll('.sheet.open')).forEach(function(sheet){
      sheet.classList.remove('open');
      sheet.setAttribute('aria-hidden', 'true');
    });

    // El panel Menú está elevado para no quedar bajo la navegación. Al cerrarlo,
    // se fuerza también su estado oculto y se reinicia su desplazamiento.
    ...
    Array.prototype.slice.call(document.querySelectorAll('.sheet-overlay')).forEach(function(overlay){
      overlay.classList.remove('active');
      overlay.style.display = 'none';
      if (overlay.dataset) overlay.dataset.closeToken = '';
    });
  } catch(e) {}

  // Restablece ventanas especiales que pueden haber cambiado su z-index.
  var editRenewalSheet = document.getElementById('editRenewalSheet');
  if (editRenewalSheet) editRenewalSheet.style.zIndex = '';
}
...
function navClients() {
  ...
  var main = document.getElementById('mainScroll');
  if (main) main.classList.add('clientsViewActive');
  filterSt = '';
  filterSvc = '';
  activeTagFilter = '';
  expandedClientCardId = '';
  ...
  var allSvc = document.querySelector('[data-svc=""]');
  if (allSvc) allSvc.classList.add('active');
  var results = document.getElementById('homeResultsBlock');
  if (results) results.style.display = 'block';
  var bar = document.getElementById('activeFilterBar');
  if (bar) bar.style.display = 'none';
  syncClientQuickFilters();
  renderTagFilterBar();
  renderCards();
...
function clientBrowserCardHtml(c, direction) {
  ...
  var heroClass = clientHasPendingPayment(c) ? 'pending' : (status === 'exp' ? 'expired' : (status === 'warn' ? 'warning' : 'active'));
  var statusLabel = clientHasPendingPayment(c) ? 'Pago pendiente' : (status === 'exp' ? 'Expirado' : (status === 'warn' ? 'Expira pronto' : 'Activo'));
  ...
  tags += '<span class="clientCleanTag days">' + esc(daysChip) + '</span>';
  ...
  if (c.avisoRenovacionContestado || c.aviso_renovacion_contestado) tags += '<span class="clientCleanTag answered">Contestó</span>';
  var r = renewalResponseValue ? renewalResponseValue(c) : (c.avisoRenovacionRespuesta || c.aviso_renovacion_respuesta || '');
  if (r === 'renueva') tags += '<span class="clientCleanTag renewYes">Renueva</span>';
  else if (r === 'no_renueva') tags += '<span class="clientCleanTag renewNo">No renueva</span>';
  else if (clientHasRenewalNotice(c) && !(c.avisoRenovacionContestado || c.aviso_renovacion_contestado)) tags += '<span class="clientCleanTag noanswer">Sin contestar</span>';

  var html = '';
  html += '<div class="clientBrowserCardShell clientCleanShell ' + heroClass + ' ' + (direction === 'prev' ? 'fromLeft' : 'fromRight') + '">';
  ...
  html +=           '<div class="clientCleanSub">' + esc(mainApp || '-') + '</div>';
  html +=         '</div>';
  html +=       '</div>';
  html +=       '<div class="clientCleanStatus ' + heroClass + '"><span>' + esc(statusLabel) + '</span><strong>' + esc(daysChip) + '</strong></div>';
  html +=     '</div>';
  html +=     '<div class="clientCleanExpiry">';
  html +=       '<div><small>Estado</small><strong>' + esc(clientBrowserStatusText(c)) + '</strong></div>';
  html +=       '<div><small>Expira</small><strong>' + formatDate(c.expiry) + '</strong></div>';
  html +=     '</div>';
...
function ensureClientMoreDeleteButton() {
  var list = document.querySelector('#clientMoreSheet .premiumOptionList');
  if (!list) return null;

  // "Editar" ya está en la ficha desplegada. En "Más" solo debe quedar borrar.
  Array.prototype.slice.call(list.querySelectorAll('button')).forEach(function(optionBtn){
    if (!optionBtn.matches('[data-client-more-delete="true"], .clientMoreDeleteBtn')) {
      optionBtn.remove();
    }
  });

  var btn = list.querySelector('[data-client-more-delete="true"], .clientMoreDeleteBtn');
  if (!btn) {
    btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'danger clientMoreDeleteBtn';
    btn.setAttribute('data-client-more-delete', 'true');
    btn.innerHTML = '<span>&#128465;</span><strong>Eliminar cliente</strong><small>Se pedirá confirmación antes de borrarlo</small>';
    btn.addEventListener('click', clientMoreDelete);
...
function openClientMoreActions(id) {
  ...
  if (title) title.textContent = c ? ('Más opciones · ' + c.name) : 'Más opciones';
  ensureClientMoreDeleteButton();
  openSheet('clientMoreSheet','clientMoreOverlay');

  // Repite la comprobación tras la animación por si el navegador restauró HTML en caché.
  setTimeout(ensureClientMoreDeleteButton, 60);
}
...
function clientMoreDelete() {
  ...
  setTimeout(function(){
    showPrettyConfirm({
      title: 'Eliminar cliente',
      message: '¿Seguro que quieres eliminar a "' + c.name + '"? Se borrará de forma permanente y esta acción no se puede deshacer.',
...
function setAuthenticationScreenLocked(locked) {
  try {
    document.body.classList.toggle('auth-locked', !!locked);
    if (locked) {
      if (typeof closeAllNavigationPanels === 'function') closeAllNavigationPanels();
      Array.prototype.slice.call(document.querySelectorAll('.sheet')).forEach(function(sheet){
        sheet.classList.remove('open');
        sheet.style.display = '';
      });
      Array.prototype.slice.call(document.querySelectorAll('.sheet-overlay')).forEach(function(overlay){
...
function showAppAfterLogin() {
  setAuthenticationScreenLocked(false);
...
function setMfaVisible(visible) {
  if (visible) setAuthenticationScreenLocked(true);
...
async function completeSupabaseLogin(fromSavedSession) {
  ...
  try {
    ...
    clearLoginLoading();
    showAppAfterLogin();
    // Acceso correcto: sin aviso emergente para mantener la pantalla despejada.
  } finally {
    hideElegantLoading();
  }
}
...
async function cancelMfaLogin() {
  setAuthenticationScreenLocked(true);
...
function clientExpiryRemainingText(expiry) {
  if (!expiry) return 'sin fecha';
  var days = getDaysLeft(expiry);
...
function openSheet(id, ovl) {
  ...
  if (sheet) {
    ...
    setTimeout(function(){
      // Evita que una ventana anterior vuelva a abrirse tras pulsar otra sección.
      if (token === sheetOpenToken && (!overlay || overlay.classList.contains('active'))) {
...
function closeSheet(id, ovl) {
  var overlay = document.getElementById(ovl);
  var sheet = document.getElementById(id);
  var closeToken = String(++sheetOpenToken);

  if (sheet) {
    sheet.classList.remove('open');
    sheet.setAttribute('aria-hidden', 'true');
    if (id === 'menuSheet') sheet.scrollTop = 0;
  }
  if (overlay && overlay.dataset) overlay.dataset.closeToken = closeToken;
...
async function editFinanceMovement(id) {
  ...
  // La categoría se conserva internamente por compatibilidad, pero ya no se pide al editar.
  var category = item.category || 'Otro';
...
function syncClientQuickFilters() {
  document.querySelectorAll('[data-client-filter]').forEach(function(btn){
    btn.classList.toggle('active', (btn.getAttribute('data-client-filter') || '') === (filterSt || ''));
  });
}

function setClientQuickFilter(st) {
  filterSt = st || '';
  expandedClientCardId = '';
  hasSelectedClientFilter = true;
  syncClientQuickFilters();
  ...
  if (!filterSt) {
    ...
    if (lbl) {
      if (filterSt === 'active_only') lbl.textContent = 'Mostrando: Clientes activos';
      if (filterSt === 'warn') lbl.textContent = 'Mostrando: Caducan pronto';
      if (filterSt === 'exp') lbl.textContent = 'Mostrando: Expirados';
      if (filterSt === 'paypend') lbl.textContent = 'Mostrando: Pendientes de pago';
    }
  }
  renderCards();
}

function toggleClientCardExpanded(id) {
  var value = String(id || '');
  expandedClientCardId = String(expandedClientCardId || '') === value ? '' : value;
  document.querySelectorAll('.clientCard-accordion').forEach(function(card){
    var isOpen = String(card.getAttribute('data-client-id') || '') === String(expandedClientCardId || '');
    card.classList.toggle('expanded', isOpen);
    var summary = card.querySelector('.clientCard-summary');
    var panel = card.querySelector('.clientCard-expand');
    if (summary) summary.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
    if (panel) panel.setAttribute('aria-hidden', isOpen ? 'false' : 'true');
  });
  if (expandedClientCardId) {
    var target = null;
    document.querySelectorAll('.clientCard-accordion').forEach(function(card){ if (String(card.getAttribute('data-client-id') || '') === value) target = card; });
    if (target) setTimeout(function(){ target.scrollIntoView({behavior:'smooth', block:'nearest'}); }, 120);
  }
}
...
function quickFilter(st) {
  ...
  var bar = document.getElementById('activeFilterBar');
  var lbl = document.getElementById('activeFilterLabel');
  var inClientsView = document.getElementById('mainScroll') && document.getElementById('mainScroll').classList.contains('clientsViewActive');
  if (bar) bar.style.display = (!st && inClientsView) ? 'none' : 'flex';
  if (lbl) {
    if (!st) lbl.textContent = 'Mostrando: Todos los clientes';
    if (st === 'active_only') lbl.textContent = 'Mostrando: Clientes activos';
...
function renderCards() {
  var searchBox = document.getElementById('searchBox');
  var search = searchBox ? searchBox.value.toLowerCase().trim() : '';
  var container = document.getElementById('cardsContainer');
  var empty = document.getElementById('emptyState');
  if (!hasSelectedClientFilter && !search) {
    if (container) container.innerHTML = '';
    if (empty) empty.style.display = 'none';
    var hiddenResults = document.getElementById('homeResultsBlock');
    if (hiddenResults) hiddenResults.style.display = 'none';
    return;
  }
  ...
  var filtered = clients.filter(function(c) {
    ...
    var appsText = (c.apps || []).map(function(a){ return [a.name,a.customName,a.mac,a.code].filter(Boolean).join(' '); }).join(' ').toLowerCase();
    var ms = !search || c.name.toLowerCase().indexOf(search)>=0 || (c.user||'').toLowerCase().indexOf(search)>=0 || (c.phone||'').toLowerCase().indexOf(search)>=0 || tagText.indexOf(search)>=0 || appsText.indexOf(search)>=0;
    var mv = !filterSvc || c.service===filterSvc;
    var mt = !filterSt || (filterSt==='attention' ? (typeof clientAttentionReasons === 'function' && clientAttentionReasons(c).length > 0) : (filterSt==='paypend' ? clientHasPendingPayment(c) : (filterSt==='advised' ? clientHasRenewalNotice(c) : (filterSt==='answered' ? clientHasAnsweredRenewalNotice(c) : (filterSt==='noanswer' ? clientHasUnansweredRenewalNotice(c) : (filterSt==='warn_no_advised' ? (getStatus(c.expiry)==='warn' && !clientHasRenewalNotice(c)) : (filterSt==='tag_no_contesta_renovar' ? (normalizeClientTags(c.tags).map(function(t){return t.toLowerCase();}).indexOf('no contesta al renovar')>=0) : (filterSt==='active_only' ? getStatus(c.expiry)==='ok' : (filterSt==='ok' ? (getStatus(c.expiry)==='ok'||getStatus(c.expiry)==='warn') : getStatus(c.expiry)===filterSt)))))))));
  ...
  if (container) container.innerHTML = '';
  var count = document.getElementById('clientsListCount');
  ...
  filtered.forEach(function(c) {
    ...
    var svcLabel = c.service === 'ESPANA' ? 'ESPAÑA' : (c.service || 'TODO');
    var cardStatus = getStatus(c.expiry);
    var visualState = clientHasPendingPayment(c) ? 'pending' : (cardStatus === 'exp' ? 'expired' : (cardStatus === 'warn' ? 'warning' : 'active'));
    var summaryStatusHtml = cardStatus === 'exp' ? '<span class="badge badgeExp">Expirado</span>' : (cardStatus === 'warn' ? '<span class="badge badgeWarn">Caduca pronto</span>' : '<span class="badge badgeOk">Activo</span>');
    var isExpanded = String(expandedClientCardId || '') === String(c.id);
    var div = document.createElement('article');
    div.className = 'clientCard clientCard-accordion clientCard-c-' + visualState + (isExpanded ? ' expanded' : '');
    div.setAttribute('data-client-id', c.id);
    div.innerHTML =
      '<button type="button" class="clientCard-summary" data-id="'+esc(c.id)+'" aria-expanded="'+(isExpanded?'true':'false')+'" onclick="toggleClientCardExpanded(this.dataset.id)">' +
        '<span class="clientCard-summaryInfo">' +
          '<strong class="clientCard-name">' + esc(c.name) + '</strong>' +
          '<small class="clientCard-app">' + esc(mainApp) + '</small>' +
        '</span>' +
        '<span class="clientCard-summaryStatus">' +
          summaryStatusHtml +
          (clientHasPendingPayment(c) ? '<span class="badge badgePayPending">Pago pendiente</span>' : '') +
        '</span>' +
        '<span class="clientCard-summaryDate"><span>' + (cardStatus === 'exp' ? 'Caducó ' : 'Caduca ') + formatDate(c.expiry) + '</span><strong>· ' + esc(clientExpiryRemainingText(c.expiry)) + '</strong></span>' +
        '<span class="clientCard-chevron" aria-hidden="true">⌄</span>' +
      '</button>' +
      '<div class="clientCard-expand" aria-hidden="'+(isExpanded?'false':'true')+'">' +
        '<div class="clientCard-detailGrid">' +
          '<div class="clientDetailBlock">' +
            '<span>Usuario</span>' +
            '<div><strong>' + esc(c.user||'-') + '</strong>' + (c.user ? '<button class="btnCopy" data-copy="'+esc(c.user)+'" onclick="copyText(this.dataset.copy,this)" aria-label="Copiar usuario">Copiar</button>' : '') + '</div>' +
          '</div>' +
          '<div class="clientDetailBlock">' +
            '<span>Clave</span>' +
            '<div><strong class="pass">' + esc(c.pass||'-') + '</strong>' + (c.pass ? '<button class="btnCopy" data-copy="'+esc(c.pass)+'" onclick="copyText(this.dataset.copy,this)" aria-label="Copiar clave">Copiar</button>' : '') + '</div>' +
          '</div>' +
          '<div class="clientDetailBlock">' +
            '<span>Teléfono</span>' +
            '<div><strong>' + esc(c.phone||'-') + '</strong>' + (c.phone ? '<button class="btnCopy" data-copy="'+esc(c.phone)+'" onclick="copyText(this.dataset.copy,this)" aria-label="Copiar teléfono">Copiar</button>' : '') + '</div>' +
          '</div>' +
        '</div>' +
        '<div class="clientCard-secondaryMeta">' +
          '<span class="clientServiceMini">' + esc(svcLabel) + '</span>' +
          (clientHasRenewalNotice(c) ? '<span class="clientCommTag advised">Avisado</span>' : '<span class="clientCommTag neutral">Sin avisar</span>') +
          (clientHasAnsweredRenewalNotice(c) ? '<span class="clientCommTag answered">Contestado</span>' : (clientHasUnansweredRenewalNotice(c) ? '<span class="clientCommTag unanswered">Sin contestar</span>' : '')) +
          renewalReplyBadgeHtml(c) +
        '</div>' +
        clientTagsHtml(c) +
        ...
        '<div class="clientCard-actions clientCard-actions-accordion">' +
          ...
          '<button class="act-more" data-id="'+esc(c.id)+'" onclick="openClientMoreActions(this.dataset.id)">&#8942; Más</button>' +
        '</div>' +
      '</div>';
...
var FIXED_IMAGE_SLOTS = [
  ...
  { key: 'pelicula_3', label: 'PELÍCULA RECOMENDADA 3', path: 'pelicula_recomendada_3_live_v2.jpg', icon: '&#127916;' },
...
var FIXED_IMAGE_SLOT_PATHS = {
  ...
  pelicula_3: 'pelicula_recomendada_3_live_v2.jpg',
...
function fixedAppImageBaseUrl(slotKey) {
  try {
    var resolvedKey = slotKey || 'horarios_mundial';

    // Película 3 se sirve mediante una Edge Function pública que fuerza una
    // lectura nueva del JPG en cada petición. Así el enlace que usan los
    // clientes sigue siendo fijo y termina en .jpg, pero no hereda la caché
    // antigua del objeto de Supabase Storage.
    if (resolvedKey === 'pelicula_3' && CONFIG.movie3ProxyUrl) {
      return String(CONFIG.movie3ProxyUrl).trim();
    }

    var slot = fixedSlotByKey(resolvedKey);
    var sb = initSupabase();
...
function renderFixedImageSlots() {
  ...
  setFixedImageStatus('Tienes 6 enlaces fijos para películas y series.', 'ok');
}
...
// ========== DETECCION AUTOMATICA DE EVENTOS PARA RESULTADOS V26 ==========
function cartEscapeHtml(value) { return String(value == null ? '' : value).replace(/[&<>"']/g,function(ch){ return ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[ch]; }); }

function cartNormalizeSportLabel(text) {
  var t = String(text || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  if (/ciclismo|cycling|tour de francia|tour de polonia|vuelta a/.test(t)) return 'Cycling';
  if (/padel|pádel|premier padel/.test(t)) return 'Padel';
  if (/tenis|tennis|atp|wta|montreal/.test(t)) return 'Tennis';
  if (/golf|wyndham|ladies european tour|liv golf/.test(t)) return 'Golf';
  if (/motociclismo|motogp|moto gp|formula 1|f1|motor/.test(t)) return 'Motorsport';
  if (/natacion|natación|acuatic|swimming/.test(t)) return 'Swimming';
  if (/baloncesto|basket|nba|wnba|acb|euroleague/.test(t)) return 'Basketball';
  if (/beisbol|béisbol|baseball|mlb/.test(t)) return 'Baseball';
  if (/hockey|nhl/.test(t)) return 'Hockey';
  if (/balonmano|handball/.test(t)) return 'Handball';
  if (/voleibol|volleyball/.test(t)) return 'Volleyball';
  if (/futbol|fútbol|football|soccer|champions|laliga|premier|serie a|bundesliga|mundial/.test(t)) return 'Football';
  return '';
}

function cartSportIcon(sport) {
...
function cartCleanParticipant(text) {
  var s = String(text || '')
    .replace(/\s+(?:📺|🗂)\s*(?:CANAL|CARPETA)?\s*:?.*$/i,'')
    .replace(/\s+[|·]\s+.*$/,'')
    .replace(/^[\s\-–—|:;,.•·▪▫►▶◆◇★⭐⚽🏀🏈⚾🏒🤾🏐🎾]+/g,'')
    .replace(/[\s\-–—|:;,.•·▪▫►▶◆◇★⭐⚽🏀🏈⚾🏒🤾🏐🎾]+$/g,'')
    .replace(/\s+/g,' ')
  ...
  // Si la cartelera incluye bandera de país antes del participante,
  // nos quedamos con el texto posterior a la ÚLTIMA bandera.
  // Ej.: "TORNEO DE MONTREAL 🇨🇿 Lehecka" -> "Lehecka".
  try {
    var flags = Array.from(s.matchAll(/[\u{1F1E6}-\u{1F1FF}]{2}\s*/gu));
    if (flags.length) {
...
function cartParseDailyEvents(text) {
  ...
  function isTimed(line) {
    return /^\s*⏰/.test(line) || /^\s*[0-2]?\d[:.][0-5]\d\b/.test(line);
  }
  function isMeta(line) {
    return /^\s*(?:📺|🗂)/.test(line);
  }
  function hasVersus(line) {
    return /(?:🆚|\bvs\.?\b|\bv\.?\b|\bcontra\b)/i.test(line);
  }

  for (var idx = 0; idx < rawLines.length; idx++) {
    var line = normLine(rawLines[idx]);
    if (!line) continue;

    // Encabezados de deporte: actualizan el contexto y no generan evento.
    if (!isTimed(line)) {
      var headerSport = cartNormalizeSportLabel(line);
      if (headerSport && line.length < 90 && !isMeta(line)) currentSport = headerSport;
      continue;
    }

    var timeMatch = line.match(/(?:^|⏰\s*)([0-2]?\d[:.][0-5]\d)/);
    if (!timeMatch) continue;
    var hora = timeMatch[1].replace('.',':');

    var title = line
      .replace(/^\s*⏰\s*[0-2]?\d[:.][0-5]\d\s*[-–—|·:]?\s*/,'')
      .replace(/^\s*[0-2]?\d[:.][0-5]\d\s*[-–—|·:]?\s*/,'')
      .trim();

    if (!title) {
      rejected.push({ linea: idx + 1, texto: line });
      continue;
    }

    // V47: una entrada puede ocupar varias líneas. Ejemplo real:
    // ⏰ 13:00 · PRETEMPORADA
    // 🇬🇧 Manchester City 🆚 🇪🇸 Atlético de Madrid
    // 📺 CANAL: ...
    // 🗂 CARPETA: ...
    // Recogemos las líneas auxiliares hasta el siguiente evento o encabezado.
    var blockLines = [line];
    var continuation = [];
    var j = idx + 1;
    for (; j < rawLines.length; j++) {
      var next = normLine(rawLines[j]);
      if (!next) continue;
      if (isTimed(next)) break;

      var nextHeaderSport = cartNormalizeSportLabel(next);
      if (nextHeaderSport && next.length < 90 && !isMeta(next) && !hasVersus(next)) {
        break;
      }
    ...
    // Saltamos las líneas de continuación ya consumidas. El encabezado o el
    // siguiente evento se procesará normalmente en la siguiente vuelta.
    if (j > idx + 1) idx = j - 1;

    // El deporte explícito del título tiene prioridad sobre un contexto antiguo.
    // Esto corrige casos como LIV GOLF / FÓRMULA 1 / PREMIER PADEL mal heredados.
    var titleSport = cartNormalizeSportLabel(title);
    var sport = titleSport || currentSport || 'Unknown';
    var type = 'event';
    var home = '';
    var away = '';

    // Buscamos el enfrentamiento tanto en la línea principal como en las líneas
    // de continuación. Así no perdemos partidos cuando la competición y los
    // equipos están en líneas separadas.
    var candidateLines = [title].concat(continuation);
    var matchText = '';
    for (var c = 0; c < candidateLines.length; c++) {
      if (hasVersus(candidateLines[c])) { matchText = candidateLines[c]; break; }
    }

    if (matchText) {
      var versus = matchText.match(/\s*(?:🆚|\bvs\.?\b|\bv\.?\b|\bcontra\b)\s*/i);
      if (versus) {
        var pos = versus.index;
        var left = matchText.slice(0, pos).trim();
        var right = matchText.slice(pos + versus[0].length).trim();

        // Si el enfrentamiento está en la misma línea que la competición,
        // retiramos prefijos tipo "Pretemporada:" o "Torneo:".
        if (left.indexOf(':') >= 0) {
          var chunks = left.split(':');
          left = chunks[chunks.length - 1].trim();
        }

        home = cartCleanParticipant(left);
        away = cartCleanParticipant(right);
        if (home.length > 1 && away.length > 1) type = 'match';
        else { home = ''; away = ''; }
      }
    }

    var sourceLine = blockLines.join(' ');
    var key = (sport + '|' + hora + '|' + title + '|' + home + '|' + away).toLowerCase();
    if (seen[key]) continue;
    seen[key] = true;

    events.push({
      id: 'm17-' + (idx + 1) + '-' + Date.now().toString(36),
      sport: sport,
      type: type,
      title: title,
      home: home,
...
async function cartSave() {
  ...
  try {
    ...
    // Así, aunque otra herramienta de Cartelera modifique después record.texto,
    // resultados.html conserva exactamente los eventos que se publicaron al
    // pulsar "Guardar y publicar".
    record.resultados_texto = texto;
    record.resultados_eventos = parsedEvents.events;
    record.resultados_fecha = record.eventos_fecha;
    record.resultados_actualizados = record.eventos_actualizados;
    ...
    if (saveRes.ok) { 
      showToast('Cartelera publicada · ' + parsedEvents.events.length + ' eventos detectados');
      cartRenderDetectedEvents(texto);
...
async function cartClear() {
  ...
  try {
    ...
    record.texto = '';
    record.eventos_datos = [];
    record.resultados_texto = '';
...
async function uploadBlobToFixedSlot(slotKey, blob, contentType){
  ...
  var res = await sb.storage.from(FIXED_IMAGE_BUCKET).upload(slot.path, blob, {
    // Las tres películas usan exactamente la misma política de caché.
    // Película 3 no recibe ya ningún tratamiento especial.
    cacheControl: '1',
    upsert: true,
...
async function movieTplUploadToFixed(slotKey, btnEl){
  // Las plantillas de películas solo pueden escribir en sus tres rutas propias.
  // Esto evita cualquier cruce accidental con SERIE 1, 2 o 3.
  var allowedMovieSlots = { pelicula_1: true, pelicula_2: true, pelicula_3: true };
  if (!allowedMovieSlots[slotKey]) {
    movieTplSetStatus('Destino de película no válido.', 'err');
    showToast('Destino de película no válido', 'error');
    return;
  }
  var slot = fixedSlotByKey(slotKey);
  if(!slot) return;
...
async function seriesTplUploadToFixed(slotKey, btnEl){
  // Las plantillas de series solo pueden escribir en sus tres rutas propias.
  var allowedSeriesSlots = { serie_1: true, serie_2: true, serie_3: true };
  if (!allowedSeriesSlots[slotKey]) {
    seriesTplSetStatus('Destino de serie no válido.', 'err');
    showToast('Destino de serie no válido', 'error');
    return;
  }
  var slot = fixedSlotByKey(slotKey);
  if(!slot) return;
