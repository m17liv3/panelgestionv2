...
async function uploadBlobToFixedSlot(slotKey, blob, contentType){
  ...
  var res = await sb.storage.from(FIXED_IMAGE_BUCKET).upload(slot.path, blob, {
    // Película 3 usa una ruta nueva y revalidación inmediata para evitar la caché
    // heredada de pelicula_recomendada_3.jpg en la aplicación de clientes.
    cacheControl: slotKey === 'pelicula_3' ? '0' : '1',
    upsert: true,
...
async function movieTplUploadToFixed(slotKey, btnEl){
  ...
  try {
    ...
    movieTplSetStatus('Subida correctamente a ' + slot.label + (slotKey === 'pelicula_3' ? ' · enlace rápido actualizado' : ''), 'ok');
